...
async function main() {
  ...
  if (!amountStr || !poolAddressStr) {
    console.log(JSON.stringify({ success: false, error: 'Usage: remove-virtual-usdc.ts <amount> <pool_address>' }));
    process.exit(1);
  }
  ...
  console.error(`📊 Before removing virtual USDC:`);
  ...
  console.error(`💰 Removing ${Number(amount.toString()) / 1e9} virtual USDC...`);

  // Call remove_virtual_usdc instruction
  const tx = await program.methods
    .removeVirtualUsdc(amount)
    .accountsPartial({
  ...
  console.error(`\n📊 After removing virtual USDC:`);
  console.error(`   Virtual USDC: ${Number(virtualUsdcAfter.toString()) / 1e9} USDC (-${((Number(virtualUsdcBefore.toString()) - Number(virtualUsdcAfter.toString())) / 1e9).toFixed(2)})`);
  console.error(`   XNT Reserve: ${Number(xntReserveAfter.toString()) / 1e9} wSOL (unchanged)`);
  console.error(`   Price: $${priceAfter.toFixed(6)} (decreased from $${priceBefore.toFixed(6)})`);

  console.log(JSON.stringify({
    success: true,
    tx,
    removed: Number(amount.toString()) / 1e9,
