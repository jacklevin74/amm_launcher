...
async function main() {
  ...
  if (!amountStr || !poolAddressStr) {
    console.log(JSON.stringify({ success: false, error: 'Usage: deposit-usdc.ts <amount> <pool_address>' }));
    process.exit(1);
  }
  ...
  console.error(`💰 Depositing ${Number(amount.toString()) / 1e9} USDC...`);

  // Call deposit_usdc_price_neutral instruction
  const tx = await program.methods
    .depositUsdcPriceNeutral(amount)
    .accountsPartial({
  ...
  console.error(`   Virtual USDC: ${Number(virtualUsdcAfter.toString()) / 1e9} USDC (decreased by ${((Number(virtualUsdcBefore.toString()) - Number(virtualUsdcAfter.toString())) / 1e9).toFixed(2)})`);
  ...
  console.log(JSON.stringify({
    success: true,
    tx,
    deposited: Number(amount.toString()) / 1e9,
